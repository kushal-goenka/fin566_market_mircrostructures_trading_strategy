/*================================================================================                               
*     Source: ../RCM/StrategyStudio/examples/strategies/SimpleSentimentStrategy/SimpleSentimentStrategy.h                                                        
...
#ifndef _STRATEGY_STUDIO_LIB_EXAMPLES_SIMPLE_SENTIMENT_STRATEGY_H_
#define _STRATEGY_STUDIO_LIB_EXAMPLES_SIMPLE_SENTIMENT_STRATEGY_H_
...
class SimpleSentiment : public Strategy {
public:
    SimpleSentiment(StrategyID strategyID, const std::string& strategyName, const std::string& groupName);
    ~SimpleSentiment();
...
private: // Helper functions specific to this strategy
    void AdjustOrder(const Instrument* instrument);

    // centralizes this strategy's policy for limit order prices
    double bid(const Instrument* instrument) { return Utilities::RoundDouble<2>(instrument->top_quote().IsValid() ?  instrument->top_quote().bid() : instrument->last_trade().price()); }
    double ask(const Instrument* instrument) { return Utilities::RoundDouble<2>(instrument->top_quote().IsValid() ? instrument->top_quote().ask() : instrument->last_trade().price()); }
...
private:
    boost::unordered_map<const Instrument*, int> position_targets_;
    double sentiment_threshold_;
    double min_buzz_;
    int min_sentiment_volume_;
    int position_size_;
    bool hold_overnight_positions_;
    bool debug_;
    bool nearing_close_time_;
};
...
extern "C" {
                                   ...
                                   const char* groupName)
    {
        if (strcmp(strategyType,GetType()) == 0) {
            return *(new SimpleSentiment(strategyID, strategyName, groupName));
        } else {
            return NULL;
        }
    }
